function main(foamset,dataset,direction,normalise)
local_init;
%%
switch foamset
    case 'foam_2010'
...
% From this point onwards we work with Q_all for estimation

% LS matrix - increased dimension does not guarantee increase in rank
...
% Compare outputs
    ...
    plot(index_test(index_plot)+File.t_0,y_model(index_plot,1),'--'); hold on;
...
Theta_test  = Betas_spl_opt*A_valid';
iTheta      = 0;
...
iTheta = 0;
figure('Name','Validation Tikhonov','NumberTitle','off','visible',visFlag);
for iFile = testFiles
    fName   = [dictFolder,'/Dict_',dataset,num2str(iFile)];
    File    = matfile(fName,'Writable',true);
    indSign = S(1:finalTerm);                                               % select the indeces of significant terms from the ordered set
